...
We will see a quick setup to configure the ``tool_git`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_git`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_git`` formulas under ``/srv/salt/tool_git/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_git/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_git/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_git/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_git/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_git`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_git`` formulas under ``/srv/salt/tool_git/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_git/parameters/roles
...
We create a configuration for the role ``tool_git/server`` in ``/srv/salt/tool_git/parameters/roles/tool_git/server.yaml``:
...
We create another configuration for the role ``tool_git/client`` in ``/srv/salt/tool_git/parameters/roles/tool_git/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_git/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_git/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_git/parameters/dns:domain/  '/srv/salt/tool_git/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-git-as-json:
      test.nop:
        - name: {{ git | json }}
...
.. code-block:: sls
    ...
    git-config-file-file-managed:
      file.managed:
        - name: {{ git.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='git-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``git`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ git['winner'] }}
